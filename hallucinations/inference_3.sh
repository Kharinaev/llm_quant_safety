...
cp logs/output_3.log logs/output_prev_3.log
> logs/output_3.log

while [ $processed_examples -lt $total_examples ]
do
    # Запускаем Python-скрипт и сохраняем вывод
    output=$(CUDA_VISIBLE_DEVICES=1 python3 quip_hotpotqa.py --model-name "/root/llm_quant_safety/quantization/models/Llama-3.1-8B-Instruct-quip-2bit" --datapath "" --savepath "/root/llm_quant_safety/data/hotpotqa/responses/parts/llama_quip2_p3.csv" --batch-size 4 | tee /dev/tty)
